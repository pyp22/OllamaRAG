#!/usr/bin/env bash
# Pilotage du service Ollama (moteur LLM GPU) de la stack ollamarag.
# Usage : ./ollama.sh [start|stop|restart|logs|status]   (défaut : start)
#
# Les trois services (ollama, open-webui, docling) partagent le même
# docker-compose.yml ; ce script n'agit QUE sur « ollama ».
#
