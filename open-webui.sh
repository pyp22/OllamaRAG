#!/usr/bin/env bash
# Pilotage du service Open WebUI (interface unique du RAG) de la stack ollamarag.
# Usage : ./open-webui.sh [start|stop|restart|logs|status]   (défaut : start)
#
# Les trois services (ollama, open-webui, docling) partagent le même
# docker-compose.yml ; ce script n'agit QUE sur « open-webui ».
# Open WebUI dépend de Docling (depends_on) : « start » démarre donc aussi
# Docling s'il ne l'est pas déjà, et suppose Ollama joignable (11434).
#
...
case "${1:-start}" in
  start)
    ...
    echo "Open WebUI démarré : http://localhost:3001"
    ;;
  stop)
    $DOCKER compose stop "$SERVICE"
    echo "Open WebUI arrêté (volume des données conservé)."
    ;;
  restart)
    ...
    echo "Open WebUI redémarré : http://localhost:3001"
    ;;
  logs)
